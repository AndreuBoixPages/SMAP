...
for k = 1:length(sites)
    try
        ...
        if isFormula
            ...
            if isfield(s, 'evaluation') && isfield(s.evaluation, 'LocMoFitGUI') && ...
               isfield(s.evaluation.LocMoFitGUI, 'allParsArg')
...
if isscalar(nums)
    ...
    % Custom thresholds: auto-add -inf before and +inf after
    edges = [-inf, sort(nums), inf];
end
end
...
pard.tBin.object  = struct('String', 'Bin edges (nm)', 'Style', 'text');
pard.tBin.position = [2, 1];
pard.tBin.Width   = 1;

pard.binSpec.object  = struct('String', '20', 'Style', 'edit');
pard.binSpec.position = [2, 2];
pard.binSpec.Width   = 2;
